            ...
            <div class="content">
                <Modal v-if="showModal">
                    <div class="modal-form">
                        ...
                        <h3 class="heading">{{ selectedService.name }}</h3>
                        <p class="paragraph" v-html="selectedService.description"></p>
                    </div>
                </Modal>
                <div class="department-wrapper" v-if="department">
                    <div class="main-department">
                        <div class="department-container">
                            <div class="cover">
                                <img :src="$file(department.image)" draggable="false" alt="article-cover" />
                            </div>
                        ...
                        <h2 v-if="department.services.length > 0">Department services</h2>
                        <div class="department-services" v-if="department.services.length > 0">
                            <div class="service" v-for="service in department.services" :key="service.id">
                                <div class="cover">
                                    <img :src="$file(service.image)" draggable="false" alt="">
                                </div>
                                <div class="description">
                                    <label class="service-name">{{ service.name }}</label>
                                    <p class="service-description" v-html="service.description"></p>
                                    <button @click="openModal(service)">Read more</button>
                                </div>
                            </div>
                        </div>
                        <h2 v-if="doctors.length > 0">Department doctors</h2>
...
export default {
    ...
    methods: {
        getDepartment(){
            this.$store.dispatch("GET_A_DEPARTMENT", this.$route.params.id)
            .then(res => {
        ...
        getDoctors(){
            ...
            .then(res => {
                this.doctors = res.data.filter(member => member.department.id == this.$route.params.id)
                this.loading = false
            })
            .catch(err => {
                this.$notify({
