<template>
  <div class="dashboardContent">
    ...
    <div class="contents">
        <Topbar />
        <Loader v-if="loading" />
        <Modal v-if="modal" :current="form" :type="modalType" @close="closeModal()" @refresh="getAllSubscribers()" />
        <div class="detailed-content">
            <div class="locator-search">
                <h1 class="locator">Subscribers</h1>
                <!-- <div class="end">
                    <div class="search">
            ...
            <div class="table" v-if="subscribers.length > 0">
                <div class="headings">
                    <h3 class="heading">Names</h3>
                    <h3 class="heading">Email</h3>
                    <h3 class="heading">Subscribed on</h3>
                    <h3 class="heading"></h3>
                </div>
                <div class="service" v-for="subscribe in subscribers" :key="subscribe.id">
                    <label class="data">{{ subscribe.name }}</label>
                    <label class="data">{{ subscribe.email }}</label>
                    <label class="data">{{ subscribe.date }}</label>
                    <div class="data actions">
                        <button class="btn del" @click="openModel('delete', subscribe)">Delete</button>
                    </div>
                </div>
            </div>
...
<script>
import Modal from "./components/SubscriberModal.vue";
export default {
    name: "AdminContacts",
    components: { Modal },
    data() {
    ...
    methods: {
        getAllSubscribers(){
            this.loading = true;
            this.$store.dispatch('GET_ALL_SUBSCRIBERS')
                .then(res => {
                    this.loading = false;
